...
#   For different levels (i.e., values of density), how often each of the 25 methods
...
ggplot_list <- lapply(1:length(level_trial_list), function(level_number){
  ...
  density <- density_vec[level_number]
  ...
  gg <- gg + ggplot2::labs(x = "Method",
                           y = "Number of wins",
                           title = paste("For density =", density))
  return(gg)
})
...
ggplot2::ggsave(plot_all, file = "~/UWBiost561/vignettes/hw4-simulation_plot.png",
